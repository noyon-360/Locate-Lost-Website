...
<div class="container mx-auto p-4 sm:p-6">
    <div class="bg-darkSlate p-6 rounded-lg shadow-lg max-w-2xl mx-auto border border-darkBlue">
        ...
        <form method="POST" action="{{ route('station.register') }}" enctype="multipart/form-data">
            @csrf
            <div class="flex flex-col sm:flex-row">
                ...
                <div class="flex-1 mb-6">
                    ...
                    <div id="station_picture_container" class="flex flex-col items-center justify-center w-full">
                        <button type="button" onclick="document.getElementById('station_picture').click()"
                            class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md shadow-md transition">
                            Choose Picture
                        </button>
                        <input id="station_picture" name="station_picture" type="file" class="hidden" accept="image/*"
                            onchange="showProfilePicture(event)" />
                    </div>
                    <div id="station_picture_preview" class="mt-2 text-center">
                        <p class="text-gray-500 mt-2">Please choose an image to upload. Allowed formats: JPEG, PNG, JPG, GIF, SVG. Maximum size: 2MB.</p>
                    </div>
                    @error('station_picture')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
            </div>
            <div class="text-center p-3">
                ...
                <p class="mt-4 text-lightGray">Already have an account? <a href="{{ route('station.login') }}" class="text-blue-500 hover:underline">Login</a></p>
            </div>
        </form>
    </div>
</div>
...
<script>
    function showProfilePicture(event) {
        ...
        const preview = document.getElementById('station_picture_preview');
        preview.innerHTML = '';
