        ...
        <section class="max-w mx-auto rounded-lg mb-8">
            <div class="gap-6">
                <div class="flex flex-col items-start py-2 space-y-4">
                    <img src="{{ $user->station_picture ? asset('storage/'.$user->station_picture) : 'https://placehold.co/128x128?text=Profile+Picture' }}"
                        alt="Profile picture of {{ $user->name }}"
                        class="w-32 h-32 rounded-full border-4 border-white shadow-lg">
                </div>

                <div>
                    <p class="text-lg font-semibold text-gray-800"><strong>Name:</strong> {{ $user->station_name }}</p>
        ...
        <section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <a href="{{ route('person.details', ['id' => $report->id]) }}"
                    ...
                    @if(!empty($report->additional_pictures) && is_array(json_decode($report->additional_pictures, true)))
                    ...
                    @else
                    <p class="text-gray-500 text-sm mt-2">No additional pictures available.</p>
                    @endif
